...
class SchoolsController extends ActiveController
{
    ...
    public function actionListParents(){
        ...
        if(!empty($getAllParents)){
            ...
            $getparentChildrenInfo = [];  $getSingleParentId = []; $getParentUserInfos = []; $i = 0;
            foreach($getAllParents as $getParent){

                $getParentUserInfos[] = User::find()->where(['id' => $getParent->parent_id])->one();

                $getSingleParentId = [];
                //var_export($getParentUserInfos[$i]['id']); exit;
                // foreach($getParentUserInfos as $getParentUserInfo){

                    //var_dump($getParentUserInfo->id); exit;
                    //foreach($getParentUserInfos as $getParentUserInfo){

                        $getAllChildren = Parents::find()->where(['parent_id' => $getParentUserInfos[$i]['id']])->all();

                        //then get all the children/student info 
                        $allChildrenInfo = [];
                        foreach($getAllChildren as $getChild){

                           $allChildrenInfo =  User::findAll(['id' => $getChild->student_id]);

                        }

                        //var_dump($allChildrenInfo); exit;
