...
class GeneralController extends ActiveController
{
    ...
    public function actionSchoolProfile()
    {
        ...
        $model = User::find()
                ->where([
                    'id' => Yii::$app->user->id,
                    'type' => SharedConstant::TYPE_SCHOOL
                ])
                ->asArray()
                ->one();

        if (!$model) {
            return (new ApiResponse)->error(null, ApiResponse::UNABLE_TO_PERFORM_ACTION, 'School profile not found');
        }

        $data = [
            'school_profile' => $model,
            'curriculum' => SchoolCurriculum::find()->where(['school_id' => $model['id']])->all(), 
            'demographics' => $this->demographics($model['id'])
        ];

        return (new ApiResponse)->success($data, ApiResponse::SUCCESSFUL, 'School profile found');
    }

    private function demographics($id)
    {
        $model = UserProfile::find()->where(['user_id' => $id]);
