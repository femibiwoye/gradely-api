...
class ProfileController extends ActiveController
{
    ...
    public function actionSchoolContact()
    {
        $model = Schools::find()
            ->select([
                'website',
            ...
            ->where(['id' => Utility::getSchoolAccess()])
            ->asArray()
            ->one();
        return (new ApiResponse)->success($model);
    }

    public function actionUpdateSchoolContact()
    {
        $model = Schools::findOne(['id' => Utility::getSchoolAccess()]);

        $form = new SchoolProfile(['scenario' => SchoolProfile::SCENERIO_EDIT_SCHOOL_CONTACT]);
        $form->attributes = Yii::$app->request->post();
        if (!$model) {
            return (new ApiResponse)->error(null, ApiResponse::UNABLE_TO_PERFORM_ACTION, 'Record not found!');
        }
        ...
        if (!$form->updateSchoolContact($model)) {
            return (new ApiResponse)->error(null, ApiResponse::UNABLE_TO_PERFORM_ACTION, 'School profile not updated');
        }
