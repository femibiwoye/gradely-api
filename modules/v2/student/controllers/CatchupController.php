...
//            ]) //to be returned
    ...
    public function actionWeeklyRecommendation()
    {
        ...
        $student_ids = ArrayHelper::getColumn(
            User::findAll(['type' => SharedConstant::TYPE_STUDENT]),
            'id'
        );

        //student_recommendations depicts the students that has recieved the weekly recommendation
        $student_recommendations = ArrayHelper::getColumn(
            Recommendations::findAll(['category' => SharedConstant::RECOMMENDATION_TYPE[SharedConstant::VALUE_ZERO]]),
            'student_id'
        );
    ...
    private function createRecommendations($recommendations, $student)
    {
        if (!empty($recommendations)) {
            ...
            try {
                ...
                if (!$this->createRecommendedTopics($recommendations, $model->id)) {
                    return false;
                }
    ...
    private function createRecommendedTopics($objects, $recommendation_id)
    {
    ...
    public function actionExistingWeeklyRecommendations()
    {
