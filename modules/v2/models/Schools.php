...
class Schools extends \yii\db\ActiveRecord
{
    ...
    public function getDemographics()
    {
        ...
        $gender = [
            'male' => $total > 0 ? count($model->andWhere(['gender' => 'male'])->all()) * 100 / $total : 44,
            'female' => $total > 0 ? count($model->andWhere(['gender' => 'female'])->all()) * 100 / $total : 21,
        ];

        $age = [
            '7-10' => $total > 0 ? count($model->andWhere(['>=', (date('Y') - date('Y', strtotime('yob' . "-" . 'mob' . "-" . 'dob'))), 7])->andWhere(['<=', (date('Y') - date('Y', strtotime('yob' . "-" . 'mob' . "-" . 'dob'))), 10])->all()) * 100 / $total : 23,
            '11-12' => $total > 0 ? count($model->andWhere(['>=', (date('Y') - date('Y', strtotime('yob' . "-" . 'mob' . "-" . 'dob'))), 11])->andWhere(['<=', (date('Y') - date('Y', strtotime('yob' . "-" . 'mob' . "-" . 'dob'))), 12])->all()) * 100 / $total : 12,
            '13-15' => $total > 0 ? count($model->andWhere(['>=', (date('Y') - date('Y', strtotime('yob' . "-" . 'mob' . "-" . 'dob'))), 13])->andWhere(['<=', (date('Y') - date('Y', strtotime('yob' . "-" . 'mob' . "-" . 'dob'))), 15])->all()) * 100 / $total : 28,
            '16+' => $total > 0 ? count($model->andWhere(['>=', (date('Y') - date('Y', strtotime('yob' . "-" . 'mob' . "-" . 'dob'))), 16])->all()) * 100 / $total : 11,
        ];
